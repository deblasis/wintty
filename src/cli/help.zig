...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        \\Usage: wintty [+action] [options]
        \\
        \\Run the Wintty terminal emulator or a specific helper action.
        \\
        \\If no `+action` is specified, run the Wintty terminal emulator.
        ...
        \\`wintty -e top` will run the `top` command inside the terminal.
        \\
