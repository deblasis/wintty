...
test "glslpp compiles CRT shader to HLSL" {
    ...
    const hlsl = try glslpp.compileGlslToHlsl(alloc, src, .fragment);
    defer alloc.free(hlsl);
    try testing.expect(hlsl.len > 0);
}

test "glslpp compiles CRT shader to MSL" {
    const testing = std.testing;
    const alloc = testing.allocator;
    ...
    const msl = try glslpp.compileGlslToMsl(alloc, src, .fragment);
    defer alloc.free(msl);
    try testing.expect(msl.len > 0);
}

test "glslpp compiles CRT shader to GLSL" {
    const testing = std.testing;
    const alloc = testing.allocator;
    ...
    const glsl = try glslpp.compileGlslToGlsl(alloc, src, .fragment);
    defer alloc.free(glsl);
    try testing.expect(glsl.len > 0);
}

test "glslpp compiles focus shader to all targets" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "glslpp rejects invalid shader" {
    const testing = std.testing;
    const alloc = testing.allocator;
    ...
    const result = glslpp.compileGlslToHlsl(alloc, src, .fragment);
