...
//
// Manages the input layout matching the CellInstance struct, and the
// constant buffer for per-frame uniforms (grid dimensions, viewport size,
// time). Shader bytecodes will be generated by HlslStep at build time.
...
// TODO: wire up HlslStep build integration to generate .cso files at build time.
// Until then, these are empty placeholders - Pipeline.init() will fail at runtime.
const vs_bytecode: []const u8 = &.{};
const ps_bytecode: []const u8 = &.{};
