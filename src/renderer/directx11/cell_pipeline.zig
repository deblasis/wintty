...
//
// Loads HLSL shaders compiled at build time (via HlslStep.zig), creates the
// input layout matching the CellInstance struct, and manages the constant
// buffer for per-frame uniforms (grid dimensions, viewport size, time).
...
// Shader bytecode, compiled from cell.hlsl by the build system (HlslStep.zig).
const vs_bytecode = @embedFile("ghostty_hlsl_cell_vs");
const ps_bytecode = @embedFile("ghostty_hlsl_cell_ps");
