...
pub fn main(minimal: std.process.Init.Minimal) !MainReturn {
    ...
    global.init(.{ .main = minimal }) catch |err| {
        ...
        //
        // It may not use `std.log`. `Logging.stderr` is false whenever
        // `app_runtime` is `none` - and that artifact calls `main` for its
        // CLI actions - so the report would be dropped in exactly the build
        // that has no other sink.
