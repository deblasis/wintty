#requires -Version 7
# Kitty graphics via palette Paste (user path). Isolated XDG.
...
function Get-WinUiWindows([uint32]$ProcId) {
    ...
    $cb = [MzKT+EnumProc]{
        param($h,$lp)
        [uint32]$o=0; [void][MzKT]::GetWindowThreadProcessId($h,[ref]$o)
        if ($o -ne $ProcId -or -not [MzKT]::IsWindowVisible($h)) { return $true }
        if ([MzKT]::ClassOf($h) -ne 'WinUIDesktopWin32WindowClass') { return $true }
        $hwnd64 = $h.ToInt64()
        $rc = [MzKT]::RectOf($hwnd64)
        if ($null -eq $rc) { return $true }
        $hits.Add([pscustomobject]@{ Hwnd64=$hwnd64; Title=[MzKT]::TitleOf($h); Area=($rc.W*$rc.Hh) })
        return $true
    }
    [void][MzKT]::EnumWindows($cb,[IntPtr]::Zero)
...
function Splash-Visible([int]$ProcId) {
    ...
    $cb = [MzKT+EnumProc]{
        param($hwnd, $lp)
        [uint32]$owner=0; [void][MzKT]::GetWindowThreadProcessId($hwnd,[ref]$owner)
        if ($owner -ne $ProcId) { return $true }
        if ([MzKT]::ClassOf($hwnd) -eq 'WinttySplash' -and [MzKT]::IsWindowVisible($hwnd)) { $script:splashSeen = $true }
        return $true
    }
    [void][MzKT]::EnumWindows($cb,[IntPtr]::Zero)
...
function Invoke-El($el, [uint32]$ProcId, [string]$what, [int64]$MainHwnd) {
    ...
    $hit = [MzKT]::ClickScreen($ProcId, $x, $y, $false)
...
function Count-TabItemsOn([int64]$Hwnd64) {
    $root = $null
    try {
        $root = [System.Windows.Automation.AutomationElement]::FromHandle([MzKT]::P($Hwnd64))
    } catch {
        return 0
    }
...
function Open-TabCloseMenu([int64]$Hwnd64, [uint32]$ProcId) {
    $rc = [MzKT]::RectOf($Hwnd64)
    $hit = [MzKT]::ClickScreen($ProcId, $rc.L + 80, $rc.T + 16, $true)
...
function Open-Palette([int64]$MainHwnd, [uint32]$ProcId) {
    ...
    if ($null -eq $pal) {
        $pt = Get-GridClickPoint $MainHwnd
        [void][MzKT]::ClickScreen($ProcId, $pt.X - 80, $pt.Y - 80, $false)
        Start-Sleep -Milliseconds 300
        $hit = [MzKT]::ClickScreen($ProcId, $pt.X, $pt.Y, $true)
...
function Assert-FgEnter([int64]$MainHwnd, [uint32]$ProcId) {
    $h = [MzKT]::P($MainHwnd)
    $fg0 = [MzKT]::GetForegroundWindow()
    [uint32]$fgPid0 = 0
    $fgTid = [MzKT]::GetWindowThreadProcessId($fg0, [ref]$fgPid0)
    $selfTid = [MzKT]::GetCurrentThreadId()
    if ($fgTid -ne 0 -and $fgTid -ne $selfTid) {
        [void][MzKT]::AttachThreadInput($selfTid, $fgTid, $true)
    }
    [void][MzKT]::BringWindowToTop($h)
    [void][MzKT]::SetForegroundWindow($h)
    if ($fgTid -ne 0 -and $fgTid -ne $selfTid) {
        [void][MzKT]::AttachThreadInput($selfTid, $fgTid, $false)
    }
    Start-Sleep -Milliseconds 200
    if (-not [MzKT]::FgIs($ProcId)) {
        $fg = [MzKT]::GetForegroundWindow()
        $name = (Get-Process -Id ([MzKT]::PidOf($fg)) -ErrorAction SilentlyContinue).ProcessName
...
$tempXdg = Join-Path $env:TEMP ("wintty-fuzz-xdg-kt-{0:HHmmss}" -f (Get-Date))
...
try {
    ...
    $hit = [MzKT]::ClickScreen($pid32, $gx, $gy, $false)
    if (-not $hit.Ok) { throw "grid click $($hit.Why)" }
    Write-Host "focused TerminalGrid $gx,$gy"
    Shot $hwnd64 '01-grid-focus'

    Set-Clipboard -Value (New-KittyCommand)
    Write-Host 'clipboard set (kitty RGBA transmit+place)'
    ...
    $hit = [MzKT]::ClickScreen($pid32, $pt.X, $pt.Y, $false)
    if (-not $hit.Ok) { throw "grid re-focus $($hit.Why)" }
    Assert-FgEnter $hwnd64 $pid32
    Start-Sleep -Milliseconds 1800
    Shot $hwnd64 '03-kitty'
    $kittyHits = Count-KittyPixels (Join-Path $OutDir 'shots\03-kitty.png')
    $kittyOk = $kittyHits -ge 8
    Write-Host "kittyHits=$kittyHits kittyOk=$kittyOk allowClicked=$allowClicked"
}
finally {
    if ($null -ne $proc) {
        $proc.Refresh()
...
if (-not $conptyPresent) { Write-Host 'PRODUCT_FAIL: conpty.dll missing next to Wintty.exe'; exit 2 }
if (-not $kittyOk) { Write-Host 'KITTY_UNVERIFIED: no #FF00CC block in grid'; exit 2 }
exit 0
