...
# The name carries padding for the same reason the hashtable fixture's does:
# both halves trim, so both halves need something that would notice if one
# stopped - and a symmetric rule with coverage on one side is how the halves
# come apart again.
@{
    layer = 'pro'
    harnesses = @(
