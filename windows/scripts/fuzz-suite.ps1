...
$Harnesses = [System.Collections.Generic.List[object]]@(
    ...
    [ordered]@{ name = 'frame-style';    script = 'frame-style-fuzz.ps1';         tags = @('chrome','render'); outDir = $true; seed = $true; minutes = 6
                oracle = 'launches once per config over a fixed spanning set - both window-theme values against all three frame-style values, frame-style unset, and a translucent frame over a solid backdrop - and judges two things. Contrast: the title row''s text against the title row''s own fill, and the tab strip''s text against its own, at WCAG 4.5:1 from a verbatim port of ThemeResolution.RelativeLuminance/ContrastRatio. Material: solid must paint different chrome from frosted for two configs differing only in frame-style, by more than the same channel delta the tab-close harness uses, and a translucent frame over a solid backdrop must paint the SAME chrome as a solid one. frosted against crystal is measured and reported, never asserted - they are one frame material by design, and so is the High Contrast pin, under which the material layer stands down and reports rather than asserting. Four controls run first and each can fail: two captures of one unchanged window differ in no chrome pixel, a selected tab row measures differently from an unselected one, the title text yields an ink/fill gap far larger than an ink-free strip of the same chrome, and that ink-free strip scores under the floor. It reads the desktop polarity and High Contrast and sets neither. A translucent frame reveals the desktop, so a failed material comparison is re-read against the screen behind the window once every window is down, and a desktop sitting at the opaque frame''s own shade is reported as indistinguishable and exits 1 rather than being filed as a defect. It samples chrome only, so a wrong terminal background passes; it compares region MEANS, so a fill that is right on average and wrong in places passes; it scores contrast off the screen rather than off the theme, so a row that is readable against a wallpaper that happens to sit behind it passes; only the VERTICAL layout is staged, so the horizontal strip is not sampled at all; and the -Random cases get the contrast layer only, because the material layer needs a matched pair of configs' }
)
