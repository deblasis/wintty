...
$Harnesses = [System.Collections.Generic.List[object]]@(
    [ordered]@{ name = 'search';         script = 'search-fuzz.ps1';               tags = @('smoke','search'); outDir = $true;  seed = $true;  minutes = 2
                oracle = 'seam-actuated, send-text armed, zero OS input: counts matches in the terminal UIA document itself; printable non-space needles are checked against that count, the rest only for a well-formed counter. The needle is set over UIA and next, previous and close are the bar''s own buttons, so key leaks through the bar and the needle box''s Enter and Escape handlers are not checked' }
    [ordered]@{ name = 'probe';          script = 'mouse-fuzz-probe.ps1';          tags = @('smoke','core');   outDir = $true;  seed = $false; minutes = 1
                oracle = 'seam-actuated, zero OS input: liveness over new tab, sidebar, select, the pane and tab menus, wheel and resize, crash.log did not grow, and two new-tab chords add two tabs' }
    [ordered]@{ name = 'tab-colors';     script = 'mouse-fuzz-tab-colors.ps1';     tags = @('tabs');           outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-actuated, zero OS input: every preset plus None, a recolor and two layout round trips, asserting the manager colour of every tab at every stop and that the layout switched; it compares no pixel, so a build that paints them all alike passes' }
    ...
    [ordered]@{ name = 'remain';         script = 'mouse-fuzz-remain.ps1';         tags = @('chrome');         outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-actuated, zero OS input: liveness plus the tab overview opening and closing on Escape; rename, color, snap, zoom, paste and quake are driven and logged but not gated, so a build missing all six still passes' }
