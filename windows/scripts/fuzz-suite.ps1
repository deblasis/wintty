...
$Harnesses = [System.Collections.Generic.List[object]]@(
    [ordered]@{ name = 'search';         script = 'search-fuzz.ps1';               tags = @('smoke','search'); outDir = $true;  seed = $true;  minutes = 2
                oracle = 'seam-actuated, send-text armed, zero OS input: counts matches in the terminal UIA document itself; printable non-space needles are checked against that count, the rest only for a well-formed counter. Next, previous and Escape go through the needle box''s own key handler and losing its focus is a finding; a wheel must move the viewport unless it is at that end. Key leaks through the bar to the shell are not checked' }
    [ordered]@{ name = 'probe';          script = 'mouse-fuzz-probe.ps1';          tags = @('smoke','core');   outDir = $true;  seed = $false; minutes = 1
                oracle = 'seam-actuated, zero OS input: liveness over the New tab button, sidebar, select, the pane and tab menus, wheel and resize, crash.log did not grow, and two New tab clicks add two tabs' }
    [ordered]@{ name = 'tab-colors';     script = 'mouse-fuzz-tab-colors.ps1';     tags = @('tabs');           outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-actuated, zero OS input: every preset plus None, a recolor and two layout round trips. Asserts the manager colour of every tab at every stop, the ink pass giving ink to tagged tabs only, inactive tabs painted apart per preset and alike per colour in a strip capture, and every tab listed in the vertical strip' }
    ...
    [ordered]@{ name = 'initial-size';   script = 'seam-initial-size.ps1';         tags = @('smoke','panes');  outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-actuated, zero OS input: for the first tab, a tab opened after the window settles, and tabs seeded behind the active one (the shape a restore has, checked once each is shown), the pixel and cell size a surface was created at, which is the size its pty starts with, must equal the size the pane settles on. A hidden tab may have no pty until it is first shown. The shell''s own first size report is recorded but not judged, because whether it lands before the first resize is a race; the display scale is recorded, and at 1.0 the scale half of the calculation is not exercised here (unit tests cover 1.5 and 2.0). The quick-terminal window, splits and daemon panes are not staged' }
    ...
    [ordered]@{ name = 'dialogs';        script = 'mouse-fuzz-dialogs.ps1';        tags = @('dialogs');        outDir = $true;  seed = $false; minutes = 2
                oracle = 'seam-actuated, zero OS input: the pane menu lists Close Pane and Change Tab Title, Search Scrollback shows the bar, the inspector toggle opens a window or its notice, About opens a window, Keyboard Shortcuts shows its search box, plus liveness' }
    ...
    [ordered]@{ name = 'undo-osc';       script = 'mouse-fuzz-undo-osc.ps1';       tags = @('vt');             outDir = $true;  seed = $false; minutes = 2
                oracle = 'seam-actuated, send-text armed: the active tab''s leaf count must go 1 -> 2 across a split and back to 1 across the undo chord (Ctrl+Shift+Z) - the old undoOk was assigned true and gated nothing; a closed tab must stay closed until the reopen chord (Ctrl+Shift+D) restores it, seeded title and all (a count alone cannot tell a restore from a fresh tab); and the OSC title the shell writes must land in the tab''s shell-reported title (the caption is masked by the seeded override), which used to print OSC_UNVERIFIED and exit 0. All gate exit 2' }
    ...
    [ordered]@{ name = 'remain';         script = 'mouse-fuzz-remain.ps1';         tags = @('chrome');         outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-actuated, zero OS input: liveness, the tab overview opening, holding focus and closing on Escape, and Tab Color opening the picker and a Teal swatch colouring the tab; rename, snap, zoom, paste and quake are driven and logged but not gated, so a build missing all five still passes' }
    ...
    [ordered]@{ name = 'jumplist';       script = 'mouse-fuzz-jumplist.ps1';       tags = @('shell');          outDir = $true;  seed = $false; minutes = 3
                oracle = 'seam-launched, zero OS input: jump-list CLI arguments reach the running primary and open what they name, across five checks; the command line is tested, not the shell''s jump list' }
    ...
    [ordered]@{ name = 'config-vanish';  script = 'seam-config-vanish.ps1';        tags = @('config');         outDir = $true;  seed = $false; minutes = 2
                oracle = 'a config file deleted while the app runs heals with ONE reload and no watcher (#1155), driven through the seam against the real ConfigService with auto-reload-config left off, zero synthesized OS input. Two launches: the watched wintty/config.wintty deleted, and one of two layered files (a migrated ghostty/config.ghostty beside it) deleted. Each asks for a High Contrast palette once, the call HighContrastMonitor makes, and requires three things: the op''s own answer shows that reload DECLINED (the live config still the file''s, no palette applied); an applied reload later carries the palette with nothing else touching the app; and that heal took at least 700ms, so a first report that was believed, the save-gap hazard the 900ms floor exists for, is a finding rather than a fast pass. The first launch then recreates the file and requires one reload to apply it; the second requires the legacy file to be what runs. Applied reloads are read from a change log the seam records when each ConfigChanged fan-out runs, because the app''s own monitor asks for no palette right after every applied reload on a desktop with High Contrast off. It says nothing about the watcher path (auto-reload-config on), a locked or unreadable file, or anything that needs the OS High Contrast actually on, and the 700ms bound is a lower bound only: a heal that waits far too long passes up to the 4s budget' }
