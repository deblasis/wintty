...
$Harnesses = [System.Collections.Generic.List[object]]@(
    ...
    [ordered]@{ name = 'tab-drag';       script = 'mouse-fuzz-tab-drag.ps1';       tags = @('tabs','motion');  outDir = $true;  seed = $true;  minutes = 4
                oracle = 'drags in both strips and reads the product''s own drag trace back: each vertical session must pair a begin with an end, must report zero leaked motions at and after its end, and must answer its release the way the motion gate says - a settle when animations are on, the pin flight for a drop in the pinned zone, and NO settle on the leg run with animations off, which the harness sets through lib/env-guard (snapshot, set, read-back, restore) and proves the app saw via the trace''s own motion=off begin line. Final orders are asserted through UIA names seeded distinct per tab in both strips, and the same gesture run twice - once with animations on, once off over a start order restored by the inverse drag - must land identical final order. The pin boundary is crossed into and released back out (stays unpinned, order intact), then released inside the zone (pins, with the flight in the trace). A click on the vertical header folds the group, and a drag released on the folded header joins the dropped tab and must re-open the group by itself. The horizontal run label is probed positive first - hover over an expanded grouped member must SHOW it, so the next assert cannot pass vacuously - and then must be gone from the UIA tree for the whole drag. The horizontal TabView drags go through TabView''s own reorder and leave no trace session, so the trace oracle covers the vertical machine only; ghosts counts mid-drag (a superseded batch still riding the newer one) are recorded, never asserted. A refused SendInput, a lost foreground, or an env restore that did not hold is exit 1; a wrong order, an unanswered drop, a settle on the off leg, or any ghost at an end is exit 2' }
    [ordered]@{ name = 'drag-filmstrip'; script = 'vtabs-drag-filmstrip.ps1';      tags = @('tabs','motion');  outDir = $true;  seed = $false; minutes = 2
                oracle = 'films one scripted vertical drag frame by frame - CopyFromScreen and the SendInput waypoints on one clock - and measures the pixels: the selected row''s accent band (row 3 keeps the selection while row 2 does the dragging, because unselected rows cannot be told apart in pixels) must rise at least 60% of a row height, must have risen 5px within 2 frames of the scheduled crossing (the gap opening), and must sit within 2px of its final position for 6 consecutive frames within 500ms of it (offsets converging), with the swapped final order read back through UIA. The crossing is measured from the earliest y the machine can register it at, so a late commit can only look worse, never better. Calibration comes from frame 0 and the harness refuses (exit 1) rather than track a guess; it also refuses on a machine whose client-area animations are off, since with them the product cuts the glide this oracle measures - the motion-on/motion-off identity pair lives in the tab-drag harness, which asserts order, while this one asserts motion. Frame PNGs are saved next to result.json' }
