...
$Harnesses = [System.Collections.Generic.List[object]]@(
    ...
    [ordered]@{ name = 'remain-title';   script = 'mouse-fuzz-remain-title.ps1';   tags = @('session');        outDir = $true;  seed = $false; minutes = 2
                oracle = 'seam-actuated: seed two tabs, split the active one, close the other, and the survivor keeps its title - the window caption must equal the survivor's title - the caption tracks the active tab, so the closed tab's title is the wrong answer - and the tab-labels op must show its shell-reported title unchanged across the close. The old pwsh-regex oracle could never match that name and was red before the migration (#964); the cross-shell case its header described was never stageable with one profile' }
...
foreach ($rel in ($dupSources.Keys | Sort-Object)) {
    ...
    while ($pending.Count -gt 0) {
        ...
        foreach ($leaf in @($dupSources[$cur])) {
            ...
            # layout-switch-filmstrip.ps1 sources C:/temp/seam-lock.ps1 -- and
